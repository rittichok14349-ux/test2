...
function DoubleRoomFemale() {
  const navigate = useNavigate();
  const { type } = useParams(); // male / female
  const { auth } = useAuth();   // จาก AuthContext
    ...
    fetch(`${import.meta.env.VITE_API_URL}/rooms`)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          // ✅ กรองตาม type (male / female)
          const filteredRooms = data.filter((room) =>
            type === "male"
              ? room.roomType === "ห้องรวม (ชาย)" || room.roomType === "male-dorm"
              : room.roomType === "ห้องรวม (หญิง)" || room.roomType === "female-dorm"
          );
...
export default DoubleRoomFemale;
